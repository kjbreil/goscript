...
func (gs *GoScript) GetState(entityId string) *state.State {
	s, _ := gs.states.Get(entityId)
	return s
}

func (gs *GoScript) GetStates(domainentity []string) *state.States {
	rtn := state.NewMultiStates(gs.states.Find(domainentity))

	return &rtn
}

func (gs *GoScript) GetDomainStates(domainentity []string) *state.States {
	rtn := state.NewMultiStates(gs.states.Find(domainentity))
